# 🌌 مستند جامع وضعیت پروژه ویپ‌لب (VAPELAB Project State & History)
**تاریخ آخرین به‌روزرسانی:** ۱۹ سپتامبر ۲۰۲۶ (۲۹ شهریور ۱۴۰۵)  
**نسخه پروژه:** ۲.۱.۰ Full-Stack (Nuxt 3 + Angular Admin + NestJS API + Drizzle ORM)

---

## 📱 قانون طلایی و ابدی پروژه: اولویت اول و مطلق حالت موبایل (Mobile-First Priority)
> [!IMPORTANT]
> **دستور قطعی کاربر و سیاست دائمی توسعه:** در تمامی مراحل طراحی، توسعه، تست و اصلاح رابط کاربری (UI/UX)، **حالت موبایل (Mobile View)** اولویت شماره ۱ است:
> ۱. تمام کامپوننت‌ها، هدر، فوتر، فرم‌ها، بنرها و صفحات باید ابتدا برای نمایشگرهای لمسی موبایل (عرض‌های ۳۷۵ الی ۵۰۰ پیکسل) بی‌نقص، بدون کراپ و ارگونومیک باشند.
> ۲. تست‌های مرورگر، بازرسی بصری و تأییدیه‌ها همواره ابتدا در ابعاد موبایل (مانند 390x844) انجام می‌پذیرد.
> ۳. نوار ناوبری پایین ([BottomNav](file:///d:/program/vape2/app/components/vapor/BottomNav.vue))، هدر بالای موبایل و شیت‌های پایین ([BottomSheet](file:///d:/program/vape2/app/components/vapor/BottomSheet.vue)) هسته تجربه کاربری اپلیکیشن هستند.

---

## 🧭 ۱. خلاصه معماری و اجزای کلیدی پروژه

پروژه **ویپ‌لب (VAPELAB)** یک اکوسیستم کامل فروشگاه تخصصی ویپ، سالت و پاد است که از ۳ بخش اصلی تشکیل شده است:

1. **فروشگاه عمومی (Storefront):**
   - **فریم‌ورک:** Nuxt 3 (Vue 3 Composition API + TypeScript)
   - **مسیر:** `/app`, `/server`, `/public`
   - **پورت اجرا:** `http://localhost:3001` (یا `3000`)
   - **ویژگی‌ها:** هویت بصری جدید VAPELAB با لوگوی اختصاصی نئونی چند مدله ([VapeLogo.vue](file:///d:/program/vape2/app/components/vapor/VapeLogo.vue))، هدر واکنش‌گرا با اولویت موبایل، استایل مدرن دارک نئونی، SSR بهینه، موتور سه‌بعدی Google Model-Viewer، سیستم احراز هویت با نام کاربری و پسورد (بدون OTP).
