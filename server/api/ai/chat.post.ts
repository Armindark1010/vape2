...
export default defineEventHandler(async (event) => {
  ...
  if (!rawQuery) {
    return {
      type: "chat_reply",
      reply: "سلام! من دستیار تخصصی ویپ‌لب هستم. چه طعم، دستگاه یا سوالی دارید تا دقیقاً راهنماییتون کنم؟ ☁️",
      products: [],
    };
  }
...
function parseUserIntent(raw: string): ParsedIntent {
  ...
  if (has("ویپورا", "ویپ لب", "ویپ‌لب", "vapora", "vapelab")) isBrand = "vapelab";
...
2. **جویس معمولی (Freebase / E-Juice):**
   ...
   - **هدف:** تولید ابر بخار حجیم، طعم‌دهی غلیظ و تجربه‌ای شبیه به قلیان.`,
    ...
    if (intent.infoTopic === "shipping") {
      return {
        type: "chat_reply",
        reply: `🚚 **شرایط و زمان‌بندی ارسال سفارشات ویپ‌لب:**
...
- 🎉 **ارسال رایگان:** برای تمامی خریدهای بالای ۶۰۰ هزار تومان ارسال کاملاً رایگان است!`,
    ...
    if (intent.infoTopic === "authenticity") {
      return {
        type: "chat_reply",
        reply: `🛡️ **تضمین ۱۰۰٪ اصالت فیزیکی و آزمایشگاهی ویپ‌لب:**
...
- سالت‌های خیلی شیرین یا استفاده مداوم با کام‌های طولانی طول عمر کویل را کاهش می‌دهد.`,
  ...
  if (intent.categorySlug === "salts" && intent.isIce) {
    ...
    aiReplyText = `اقتصادی‌ترین و باارزش‌ترین محصولات موجود در انبار ویپ‌لب با **ضمانت کامل اصالت و قیمت رقابتی**:`;
  } else if (intent.isBestSeller) {
    aiReplyText = `پرفروش‌ترین و محبوب‌ترین انتخاب‌های کاربران ویپ‌لب بر اساس بالاترین امتیاز رضایت خریداران ⭐:`;
  } else if (intent.isBrand) {
    aiReplyText = `محصولات اورجینال و پرطرفدار برند معتبر **${topRecs[0]?.product.brand || "ویپ‌لب"}** با هولوگرام اصالت:`;
  } else {
